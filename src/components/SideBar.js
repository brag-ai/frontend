...
import LoginManagerInstance from "../app/auth/loginManager";
import Link from "next/link";
import ChatButton from "@/components/ChatButton";

const SideBar = (props) => {
    const [chats, setChats] = useState([{ chatName: "chatName1" }, { chatName: "chatName2" }]);
    const router = useRouter();
    useEffect(() => {
        //make api call 

        if (props.chat) setChats(props.chat);
        return () => { };
    }, []);
    ...
    const handleLogout = (event) => {
        ...
        const loggedOut = LoginManagerInstance.logout();
        if (loggedOut) {
            router.push("/login");
    ...
    return <div className="dark hidden flex-shrink-0 bg-gray-900 md:flex md:w-[260px] md:flex-col">
            ...
            <div className="scrollbar-trigger flex h-full w-full flex-1 items-start border-white/20">
                <nav className="flex h-full flex-1 flex-col space-y-1 p-2">
                    ...
                    <div className="flex-col flex-1 overflow-y-auto border-b border-white/20">
                        {chats && chats.map((ell) => <ChatButton
                            name={ell.chatName}
                        />)}
                    </div>
                    ...
                    <Link className="flex py-3 px-3 items-center gap-3 rounded-md hover:bg-gray-500/10 transition-colors duration-200 text-white cursor-pointer text-sm"
